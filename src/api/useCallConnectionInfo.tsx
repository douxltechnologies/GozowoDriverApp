...
export const useCallConnectionInfo = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<ErrorObject | null>(null);

  const getCallConnection = async (token: string) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}api/SignalR/connection-info`,
        {
          headers: {
  ...
  return { getCallConnection, loading, error };
};
