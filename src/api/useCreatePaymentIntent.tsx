...
export const useCreatePaymentIntent = () => {
  ...
  const createPaymentIntent = async (token: string, data: CreateMobilePaymentRequest) => {
    setLoading(true);
    setError(null);
    setSuccess(null);
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}api/Payment/CreateMobilePaymentIntentAsync`,
        formData,
        {
          headers: {
      ...
      if (response.status >= 200 && response.status < 300) {
        console.log('Intent done successfully:', response.data);
        const successMsg =
          response.data?.message || 'Intent done successfully!';
  ...
  return { createPaymentIntent, loading, error, success };
};
